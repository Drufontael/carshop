...
public class Placa {
    ...
    private void validarPlaca(String placa) throws RuntimeException {
        if (placa == null) {
            throw new RuntimeException("Placa inválida: null");
        }
        String placaLimpa = placaFormat(placa);
        if (placaLimpa.length() != 7) {
            throw new RuntimeException("Placa inválida: " + placa + " (deve ter 7 caracteres)");
        }
        boolean valida = PLACA_ANTIGA.matcher(placaLimpa).matches() ||
                PLACA_MERCOSUL.matcher(placaLimpa).matches();
        if (!valida) {
            throw new RuntimeException("Placa inválida: " + placa + " (formato não reconhecido)");
        }
    }
