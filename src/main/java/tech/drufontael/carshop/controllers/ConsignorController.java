...
public class ConsignorController implements ConsignorDoc {
    ...
    public ResponseEntity<Consignor> create(ConsignorDto dto) {
        var consignor=dto.toConsignor();
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(consignor));
    }

    @GetMapping("/{register}")
    public ResponseEntity<Consignor> findByRegister(@PathVariable String register) {
        var consignor=service.read(register);
        consignor.add(linkTo(methodOn(ConsignorController.class).findAll()).withRel("Consignors list"));
    ...
    public ResponseEntity<List<Consignor>> findAll() {
        ...
        for(Consignor consignor:list){
            consignor.add(linkTo(methodOn(ConsignorController.class).findByRegister(consignor.getRegister())).withSelfRel());
        }
        return ResponseEntity.ok(list);
    }

    @PutMapping("/{register}")
    public ResponseEntity<Consignor> update(@PathVariable String register, @RequestBody ConsignorDto obj) {
